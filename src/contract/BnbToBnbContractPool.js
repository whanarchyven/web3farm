...
class BnbToBnbContractPool {
    ...
    async userData() {
        const accounts = await web3.eth.getAccounts();
        const result = await this.staking.methods.userData(accounts[0]).call((err, res) => {
            console.log(res)
        });
        return result
    }
    ...
    async viewUnpaid() {
        const accounts = await web3.eth.getAccounts();
        const result = await this.staking.methods.viewUnpaid(accounts[0]).call();
        return result
    }

    //Staked Token Methods

    async mint(amount) {
        const accounts = await web3.eth.getAccounts();
        await this.stakedToken.methods.mint(amount).send({from: accounts[0]})
    }
    ...
    async mintBooster(boosterId, amount) {
        const accounts = await web3.eth.getAccounts();
        const price = await this.mintPrice(boosterId);
        await this.erc1155.methods.mint(boosterId, amount).send({from: accounts[0], value: price}, (err, res) => {
            alert('Minted!')
        })
    }
...
export default BnbToBnbContractPool